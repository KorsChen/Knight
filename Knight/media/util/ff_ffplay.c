...
static void decoder_init(FFDecoder *d, AVCodecContext *avctx, PacketQueue *queue, SDL_cond *empty_queue_cond) {
    memset(d, 0, sizeof(FFDecoder));
    d->avctx = avctx;
    d->queue = queue;
...
static int decoder_decode_frame(FFPlayer *ffp, FFDecoder *d, AVFrame *frame, AVSubtitle *sub) {
...
static void decoder_destroy(FFDecoder *d) {
...
static void decoder_abort(FFDecoder *d, FrameQueue *fq)
{
...
//最后渲染图像的方法为video_image_display2
...
//        当前主时钟源不是video，则计算当前视频时钟与主时钟的差值：
...
//            经过compute_target_delay方法，计算出显示当前帧需要等待的时间
            ...
            if (time < is->frame_timer + delay) {
                //frame_timer + delay实际上就是当前这一帧的播放时间，如果系统时间还没有到当前这一帧的播放时间，直接跳转至display
...
/**
 如果当前这一帧的播放时间已经过了，并且其和当前系统时间的差值超过了AV_SYNC_THRESHOLD_MAX
 ，则将当前这一帧的播放时间改为系统时间，并在后续判断是否需要丢帧，其目的是为后面帧的播放时间重新调整frame_timer，
...
display:
//      如果is->force_refresh变量为0，不显示当前帧，进入video_refresh_thread中下一次循环，并睡眠等待
        /* display picture */
...
static int decoder_start(FFDecoder *d, int (*fn)(void *), void *arg, const char *name)
{
...
static int audio_open(FFPlayer *opaque, int64_t wanted_channel_layout, int wanted_nb_channels, int wanted_sample_rate, struct AudioParams *audio_hw_params)
{
    ...
    /**
     audio_open中配置了音频输出的相关参数SDL_AudioSpec，
     通过SDL_AoutOpenAudio函数设置给了Audio Output, iOS平台上即为AudioQueue
...
static int stream_component_open(FFPlayer *ffp, int stream_index)
{
    ...
    case AVMEDIA_TYPE_AUDIO:
        ...
        /*
         ...
         在audio_open中配置了音频输出的相关参数SDL_AudioSpec
         **/
...
//            通过ffpipeline_open_video_decoder创建KFF_Pipenode
...
static VideoState *stream_open(FFPlayer *ffp, const char *filename, AVInputFormat *iformat)
{
    ...
    /* start video display */
    //创建存放video解码前数据的videoq，解码后数据的pictq
    if (frame_queue_init(&is->pictq, &is->videoq, ffp->pictq_size, 1) < 0)
        goto fail;
    //创建存放audio解码前数据的audioq，解码后数据的sampq
    if (frame_queue_init(&is->sampq, &is->audioq, SAMPLE_QUEUE_SIZE, 1) < 0)
        goto fail;
    /*
     创建存放字幕解码前数据的subtitleq，解码后数据的subpq
     subtitle是与video、audio平行的一个stream，ffplay中也支持对它的处理，即创建存放解码前后数据的两个queue，并且当文件中存在subtitle时，还会启动subtitle的解码线程，由于篇幅有限，本文暂时忽略对它的相关介绍。
     **/
